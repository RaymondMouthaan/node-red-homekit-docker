...
docker_build_node_v8() {
    ...
    docker build --no-cache --build-arg BASE_IMAGE=raymondmm/node-red --build-arg QEMU_ARCH=x86_64 --file ./.docker/Dockerfile.alpine-tmpl --tag $IMAGE:build-8-alpine-amd64 .
...
docker_tag_node_v8() {
    ...
    docker tag $IMAGE:build-8-alpine-amd64 $IMAGE:$NODE_RED_HOMEKIT_VERSION-8-alpine-amd64
...
docker_push_node_v8() {
    ...
    docker push $IMAGE:$NODE_RED_HOMEKIT_VERSION-8-alpine-amd64
...
docker_manifest_list_node_v8() {
    ...
    docker manifest create $IMAGE:$NODE_RED_HOMEKIT_VERSION-8 \
      $IMAGE:$NODE_RED_HOMEKIT_VERSION-8-alpine-amd64
      # \
