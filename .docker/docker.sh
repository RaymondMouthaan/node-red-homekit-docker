...
docker_build_node_v8() {
    ...
    docker build --no-cache --build-arg BASE_IMAGE=nodered/node-red-docker --build-arg QEMU_ARCH=x86_64 --file ./.docker/Dockerfile.debian-tmpl --tag $IMAGE:build-8-debian-amd64 .
...
docker_test_node_v8() {
    ...
    docker run -d --name=test-8-debian-amd64 $IMAGE:build-8-debian-amd64
    if [ $? -ne 0 ]; then
        echo "ERROR: Docker container failed to start for build-8-debian-amd64."
        exit 1
    fi
    docker stop test-8-debian-amd64 && docker rm test-8-debian-amd64
...
docker_tag_node_v8() {
    ...
    docker tag $IMAGE:build-8-debian-amd64 $IMAGE:latest-8-debian-amd64
    docker tag $IMAGE:build-8-debian-amd64 $IMAGE:$NODE_RED_HOMEKIT_VERSION-8-debian-amd64
...
docker_manifest_list_node_v8() {
    # Manifest Create v8
    docker manifest create $IMAGE:$NODE_RED_HOMEKIT_VERSION-8 \
      $IMAGE:$NODE_RED_HOMEKIT_VERSION-8-debian-amd64
      # \
    ...
    # Manifest Push v8
    docker manifest push $IMAGE:$NODE_RED_HOMEKIT_VERSION-8
}
...
docker_manifest_list_default() {
    ...
    # docker manifest annotate $IMAGE:$NODE_RED_HOMEKIT_VERSION $IMAGE:$NODE_RED_VERSION-8-debian-arm32v7 --os=linux --arch=arm --variant=v7
