...
docker_build_node_v8() {
    ...
    docker build --no-cache --build-arg NODE_RED_IMAGE=$NODE_RED_IMAGE --build-arg NODE_RED_IMAGE_TAG=0.18.4-8-alpine-amd64  --build-arg QEMU_ARCH=x86_64 --file ./.docker/Dockerfile.alpine-tmpl --tag $IMAGE:build-$NODE_RED_VERSION-alpine-amd64 .
...
docker_tag_node_v8() {
    ...
    docker tag $IMAGE:build-$NODE_RED_VERSION-alpine-amd64 $IMAGE:$VERSION-$NODE_RED_VERSION-alpine-amd64
...
docker_push_node_v8() {
    ...
    docker push $IMAGE:$VERSION-$NODE_RED_VERSION-alpine-amd64
...
docker_manifest_list_node_v8() {
    # Manifest Create v8
    docker manifest create $IMAGE:$VERSION-$NODE_RED_VERSION \
      $IMAGE:$VERSION-$NODE_RED_VERSION-alpine-amd64
      # \
    ...
    # Manifest Push v8
    docker manifest push $IMAGE:$VERSION-$NODE_RED_VERSION
}
